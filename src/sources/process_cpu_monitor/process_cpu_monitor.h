...
// Monitors per-process and per-thread CPU utilization via /proc.
// Produces htop-style metrics: user%, sys%, total%, plus context switches,
// thread count, RSS, and optional thread-level detail for hot processes.
...
public:
    ...
    StatusOr<DataBatchPtr> Collect() override {
        ...
        for (auto& rp : ranked) {
            ...
            if (expand_threads) {
                CollectThreads(batch, proc.pid, delta_total);
            }
        }
