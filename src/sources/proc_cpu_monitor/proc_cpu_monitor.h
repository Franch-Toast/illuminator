...
class ProcCpuMonitor : public SourcePlugin {
public:
    const char* Name() const override { return "proc_cpu_monitor"; }
    ...
    Status Init(const ConfigValue& config) override {
        ...
        thread_detail_threshold_pct_ = config["thread_detail_threshold_pct"].AsDouble(10.0);
    ...
    StatusOr<DataBatchPtr> Collect() override {
        ...
        for (auto& proc : processes) {
            ...
            if (it != prev_snapshots_.end()) {
                auto& prev = it->second;
                uint64_t d_utime = proc.utime > prev.utime ? proc.utime - prev.utime : 0;
                uint64_t d_stime = proc.stime > prev.stime ? proc.stime - prev.stime : 0;
                pw.cpu_user_pct = 100.0 * static_cast<double>(d_utime) /
                                  static_cast<double>(delta_total) * num_cpus;
                pw.cpu_sys_pct = 100.0 * static_cast<double>(d_stime) /
                                 static_cast<double>(delta_total) * num_cpus;
                pw.cpu_total_pct = pw.cpu_user_pct + pw.cpu_sys_pct;
                pw.has_prev = true;
            }
            ranked.push_back(pw);
        }
        ...
        for (auto& pw : ranked) {
            ...
            rec.SetField(batch->InternString("cpu_user_pct"), pw.cpu_user_pct);
            rec.SetField(batch->InternString("cpu_sys_pct"), pw.cpu_sys_pct);
            rec.SetField(batch->InternString("cpu_total_pct"), pw.cpu_total_pct);
...
private:
    ...
    void CollectThreads(DataBatchPtr& batch, uint32_t pid,
                        uint64_t delta_total, int num_cpus) {
    ...
    double thread_detail_threshold_pct_ = 10.0;
...
IL_REGISTER_SOURCE("proc_cpu_monitor", ProcCpuMonitor);
