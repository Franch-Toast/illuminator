...
function transformMemoryUtilization(batch: DataBatch): { point: MemoryDataPoint; summary: MemorySummary } | null {
...
export function useMemoryUtilization(active = true) {
  ...
  const { latest } = useDataSource({
    ...
    transform: transformMemoryUtilization,
...
function transformMemoryProcesses(batch: DataBatch, historyMap: Map<number, number[]>): MemoryProcess[] | null {
  const records = extractRecords(batch.data)
  if (records.length === 0) return null
...
export function useMemoryProcesses(active = true) {
  ...
  const { latest } = useDataSource({
    feature: 'memory_processes',
    transform: useCallback((batch: DataBatch) => transformMemoryProcesses(batch, historyMapRef), [historyMapRef]),
    active: active && mode !== 'paused',
  })
