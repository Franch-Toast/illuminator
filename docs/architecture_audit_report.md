...
| 模块 | 评分 | 一句话评价 |
|------|------|----------|
| 核心引擎层 | **B+** | 架构清晰，Pipeline v3 设计成熟；存在 sink 列表线程安全和关机顺序问题 |
...
| # | 问题 | 模块 | 影响 |
|---|------|------|------|
| 1 | **Pipeline sink 列表线程安全**: `GetSinks()` 和 `Reconfigure()` 未加锁，与 `AddSinkRuntime` 竞态 | 引擎 | 数据竞争，可能崩溃 |
...
| 11 | **Push 模式 per-event DataBatch**: 每个事件创建一个 batch，高频场景分配压力大 | 插件 | 性能瓶颈 |
| 12 | **前端页面模式不统一**: CPU 用 FeaturePageTemplate，其他页面各自手写 shell | 前端 | 维护负担 |
| 13 | **core 层依赖 plugin 层**: `feature_driver.h` 直接 include recording_sink.h | 引擎 | 分层违反 |
| 14 | **SchedAnalyzerDriver 元数据错误**: `supports_pull=false` 但默认是 Pull 模式 | 插件 | 前端自动发现误标 |
| 15 | **SSE syncSubscription bug**: 始终发送 add，从不 remove，导致服务端过滤泄漏 | 前端 | 退订无效 |

### P2 — 建议修复（死代码/清理）

| # | 问题 | 模块 | 影响 |
|---|------|------|------|
| 16 | **`bpf_program_manager.h` 废弃**: 在 BUILD 中但无处引用 | 死代码 | 占空间，误导 |
| 17 | **`wasm_runtime.h` 空壳**: 不在任何 BUILD target 中 | 死代码 | 增加审计范围 |
| 18 | **`ebpf_sched_tracer` 孤儿插件**: 注册了但无 FeatureDriver，与 sched_analyzer 重叠 | 死代码 | 混乱 |
| 19 | **`CpuStatsAggregator` 无 Feature 集成**: 注册了但无 Driver 使用 | 死代码 | 代码质量误导 |
| 20 | **`SinkFanout` 未链接到生产二进制**: 有 BUILD 但 builtin_plugins.cc 不 include | 死代码 | 运行时不可用 |
...
**问题**:
- `Pipeline::GetSinks()` 返回 sinks 列表时无锁保护，与 `AddSinkRuntime` 竞态
- `EngineConfig` 与 `InfrastructureConfig` 存在配置重复
...
- eBPF Skeleton 模板（Push/Pull）是最好的抽象，67行即可实现完整插件
...
- v2 路由内联在 main.cc，不可单独测试
- 认证启用后 SSE 不可用（EventSource 无法发 Authorization header）
...
| SseSink 序列化 vs json_serializer.h | `sse_handler.h`, `json_serializer.h` | 统一为 `SerializeFeatureBatch()` |
| 前端 useMetricsData/History vs useDataSource | hooks/ | 弃用旧 hook |
| 前端三层缓冲 | dataBus + hooks + TimeSeriesBuffer | 统一为 DataBus ring + hook state |
...
| 运行时动态组合 Pipeline | ❌ 已移除 | YAML pipelines 不再运行 |
...
| 优先级 | 行动 | 预期效果 |
|--------|------|---------|
| P0-1 | **修复 Pipeline sink 列表线程安全** | 消除数据竞争 |
| P0-2 | **重写 README.md** | 准确描述当前架构 |
| P0-3 | **实现或删除 `/api/v1/query`** | QueryConsole 可用 |
| P0-4 | **解决 Auth+SSE 不兼容** | 认证可真正使用 |
| P1-1 | **统一 eBPF 插件基类**（参见 ebpf_plugin_redesign.md） | 消除三种实现模式 |
| P1-2 | **提取 v2 路由到独立模块** | 可测试性 |
| P1-3 | **Push 模式批量排空 ring buffer** | 性能提升 + 消除 poll 线程 |
| P1-4 | **统一前端页面模板** | 减少页面维护负担 |
| P2-1 | **清理死代码**（wasm/bpf_program_manager/fanout/sched_tracer） | 代码干净度 |
| P2-2 | **清理 YAML 配置**（删除无效键或实现它们） | 运维不被误导 |
